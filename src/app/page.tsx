...
// if(loadData){
//   setHeader(medicineType[0].medicines[0].medicine)
// }
...
useEffect(()=>{
  if(loadData){
    ...
    let dataAny=[]
    let key=0
    for(let i=0;i<medicineType[0].medicines[0].pricing.length;i++){
      for(let j=0;j<medicineType[0].medicines[0].pricing[i].type.length;j++){
        let innerData=medicineType[0].medicines[0].pricing[i].type[j]
        key=key+1;
        dataAny.push({key:key,size:innerData.size,quantity:innerData.quantity,price:innerData.price})
      }
    }
    console.log('dataAny',dataAny)
    setTableData(dataAny)
  }
  
},[medicineType[0].medicines[0],loadData])
...
const columns = [
  ...
  },
  {
    ...
    dataIndex: '',
    key: 'x',
    render: (text:any, record:any, index:any) => <a onClick={(e)=>redirectToPay({medicine:header,Size:tabelData[index].size,Price:tabelData[index].price,Quantity:tabelData[index].quantity})}>Buy</a>,
  },
];
      ...
      <div className={styles.hero_area}>
            ...
            <Container>
              <Row>
                <Col md={{ span: 6, offset: 3 }}>
                  <>
                    {
                      medicineType[0].medicines[0].pricing.map(function(price,index) {
                        return (
                          <div className={styles.pricing_section} key={index}>
                            <>
                              {
                                price.type.map((type:any,i)=>{
                                  return(
                                    <div key={i}>
                                      <div>
          ...
          <div className={styles.site_section}>
            ...
            <Row>
                <Col md={{ span: 6, offset: 3 }}>
                  <>
                    {medicineType[0].medicines[0].about.map((x,index)=>{
                      return (
                        <p key={index}>{x}</p>
                      )
                    })}
          ...
          <div className={styles.site_section}>
            <Container>
              <Row>
                <Col md={{ span: 6, offset: 3 }}>
                  <>
                    {medicineType.map(function(data,idn) {
                      return (
                        <div key={idn} className={styles.category_section}>
                          ...
                          <div className={styles.medicineType}>
                            <>
                              {
                                data.medicines.map((medicine,mi)=>{
                                  return (
                                    <div key={mi} className={styles.medicineSubType}>
